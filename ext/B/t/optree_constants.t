...
for $func (sort keys %$want) {
    ...
    checkOptree ( name    => "call $func",
		  ...
		  bc_opts => '-nobanner',
		  expect  => <<EOT_EOT, expect_nt => <<EONT_EONT);
4  <1> leavesub[2 refs] K/REFC,1 
3     <\@> lineseq KP 
1        <;> dbstate(main 833 (eval 44):1) v 
2        <\$> const[$want->{$func}[0] $want->{$func}[1]] s* 
EOT_EOT
# 4  <1> leavesub[1 ref] K/REFC,1 
# 3     <@> lineseq KP 
# 1        <;> nextstate(main 918 (eval 25):1) v 
# 2        <\$> const($want->{$func}[0] $want->{$func}[1]) s* 
EONT_EONT
...
checkOptree ( name	=> 'myyes() as coderef',
	      code	=> sub () { 1==1 },
	      noanchors => 1,
              todo      => "constant sub generated by constant folding",
	      expect	=> <<'EOT_EOT', expect_nt => <<'EONT_EONT');
...
# 4  <1> leavesub[2 refs] K/REFC,1 
# 3     <@> lineseq K 
# 1        <;> nextstate(constant 61 constant.pm:118) v:*,& 
# 2        <0> padav[@list:FAKE:m:96] 
EOT_EOT
# 4  <1> leavesub[2 refs] K/REFC,1 
# 3     <@> lineseq K 
# 1        <;> nextstate(constant 774 constant.pm:134) v:*,& 
# 2        <0> padav[@list:FAKE:m:71] 
EONT_EONT
...
# 9     <@> lineseq KP 
# 1        <;> nextstate(main 801 optree_constants.t:207) v 
# 8        <@> prtf sK 
# 3           <@> stringify[t1] sK/1 
# 2              <$> const[PV "myint %d mystr %s myfl %f pi %f\n"] s 
# 4           <$> const[IV 42] s* 
# 5           <$> const[PV "hithere"] s* 
# 6           <$> const[NV 1.414213] s* 
# 7           <$> const[NV 3.14159] s* 
EOT_EOT
# a  <1> leavesub[1 ref] K/REFC,1 
# 9     <@> lineseq KP 
# 1        <;> nextstate(main 801 optree_constants.t:208) v 
# 8        <@> prtf sK 
# 3           <@> stringify[t1] sK/1 
# 2              <$> const(PV "myint %d mystr %s myfl %f pi %f\n") s 
# 4           <$> const(IV 42) s* 
